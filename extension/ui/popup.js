...
import {
    ...
    apiSignup,
    saveBookingToServer,
    analyzeBookingScreenshot,   // ⬅️ we’ll move this into api.js
  } from "./api.js";
...
async function handleLogin() {
    ...
    try {
      ...
      console.log(result)
      // Assume backend returns { token, user: { email } }
      const token = result.token;
      const userEmail = result.user?.email || email;
  
      await setAuth(token, userEmail);
  
      try {
        updateProfileDisplay(userEmail);
      } catch (e) {
        console.warn("Profile display error (non-critical):", e);
      }
      ...
      if (loginBtn) {
        ...
        loginBtn.innerHTML = originalHtml ||
          '<span class="btn-icon"><svg xmlns="http://www.w3.org/2000/svg" ...></svg></span> Log in';
      }
    }
}
...
async function handleSignup() {
    ...
    const confirmPassword = (signupConfirmInput && signupConfirmInput.value) || "";
    ...
    try {
      if (signupBtn) {
        ...
        originalHtml = signupBtn.innerHTML;
      ...
      if (signupBtn) {
        ...
        signupBtn.innerHTML = originalHtml ||
          '<span class="btn-icon"><svg xmlns="http://www.w3.org/2000/svg" ...></svg></span> Sign up';
      }
    }
  }  
